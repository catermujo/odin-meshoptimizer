...
foreign lib {
    ...
    generateVertexRemapCustom :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, callback: proc "c" (_: rawptr, _: u32, _: u32) -> i32, _context: rawptr) -> uint ---
	...
	destination must contain enough space for the resulting remap table (vertex_count elements)
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    generatePositionRemap :: proc(destination: [^]u32, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) ---
	...
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    generateAdjacencyIndexBuffer :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) ---
	...
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    generateTessellationIndexBuffer :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) ---
    ...
    optimizeOverdraw :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, threshold: f32) ---
	...
	*/
    simplify :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, target_index_count: uint, target_error: f32, options: Simplify, result_error: [^]f32) -> uint ---
	...
	options must be a bitmask composed of meshopt_SimplifyX options; 0 is a safe default
	result_error can be NULL; when it's not NULL, it will contain the resulting (relative) error after simplification */
    simplifyWithUpdate :: proc(indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, vertex_attributes: [^]f32, vertex_attributes_stride: uint, attribute_weights: [^]f32, attribute_count: uint, vertex_lock: [^]SimplifyVertex, target_index_count: uint, target_error: f32, options: Simplify, result_error: [^]f32) -> uint ---
	...
	target_error represents the error relative to mesh extents that can be tolerated, e.g. 0.01 = 1% deformation; value range [0..1]
	result_error can be NULL; when it's not NULL, it will contain the resulting (relative) error after simplification */
    simplifySloppy :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, vertex_lock: [^]SimplifyVertex, target_index_count: uint, target_error: f32, result_error: [^]f32) -> uint ---
	...
	vertex_positions should have float3 position in the first 12 bytes of each vertex
	target_error represents the error relative to mesh extents that can be tolerated, e.g. 0.01 = 1% deformation; value range [0..1] */
    simplifyPrune :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, target_error: f32) -> uint ---
    ...
    simplifyPoints :: proc(destination: [^]u32, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, vertex_colors: [^]f32, vertex_colors_stride: uint, color_weight: f32, target_vertex_count: uint) -> uint ---
	...
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    analyzeOverdraw :: proc(indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) -> Overdraw_Statistics ---

    /* Coverage analyzer
	Returns coverage statistics (ratio of viewport pixels covered from each axis) using a software rasterizer

	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    analyzeCoverage :: proc(indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) -> Coverage_Statistics ---
    ...
    buildMeshlets :: proc(meshlets: [^]Meshlet, meshlet_vertices: [^]u32, meshlet_triangles: [^]u8, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, max_vertices: uint, max_triangles: uint, cone_weight: f32) -> uint ---
    ...
    buildMeshletsFlex :: proc(meshlets: [^]Meshlet, meshlet_vertices: [^]u32, meshlet_triangles: [^]u8, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, max_vertices: uint, min_triangles: uint, max_triangles: uint, cone_weight: f32, split_factor: f32) -> uint ---
    ...
    buildMeshletsSpatial :: proc(meshlets: [^]Meshlet, meshlet_vertices: [^]u32, meshlet_triangles: [^]u8, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, max_vertices: uint, min_triangles: uint, max_triangles: uint, fill_weight: f32) -> uint ---
    ...
    computeClusterBounds :: proc(indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) -> Bounds ---
    computeMeshletBounds :: proc(meshlet_vertices: [^]u32, meshlet_triangles: [^]u8, triangle_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) -> Bounds ---
    ...
    partitionClusters :: proc(destination: [^]u32, cluster_indices: [^]u32, total_index_count: uint, cluster_index_counts: [^]u32, cluster_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, target_partition_size: uint) -> uint ---
	...
	destination must contain enough space for the resulting remap table (vertex_count elements)
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    spatialSortRemap :: proc(destination: [^]u32, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) ---
	...
	destination must contain enough space for the resulting index buffer (index_count elements)
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    spatialSortTriangles :: proc(destination: [^]u32, indices: [^]u32, index_count: uint, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint) ---
	...
	vertex_positions should have float3 position in the first 12 bytes of each vertex */
    spatialClusterPoints :: proc(destination: [^]u32, vertex_positions: rawptr, vertex_count: uint, vertex_positions_stride: uint, cluster_size: uint) ---
